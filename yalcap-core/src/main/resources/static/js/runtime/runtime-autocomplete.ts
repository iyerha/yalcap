...
if (runtimeNamespace && runtimeNamespace.initialized) {
    throw new Error('Autocomplete runtime already initialized');
}
...
function bindAutocompleteAll(): void {
...
const runtimeAutocompleteModule: RuntimeAutocompleteModule = {
    initialized: true,
    bindAll: bindAutocompleteAll
};

(window as any).runtimeAutocomplete = runtimeAutocompleteModule;
(window as any).autocompleteRuntime = runtimeAutocompleteModule;

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bindAutocompleteAll);
} else {
    bindAutocompleteAll();
}

document.addEventListener('htmx:afterSwap', bindAutocompleteAll);
